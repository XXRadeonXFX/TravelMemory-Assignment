...
existing_instances = ec2.describe_instances(
    Filters=[
        {'Name': 'tag:Name', 'Values': ['TM-Prince-Frontend']},
...
if existing_instances['Reservations']:
    ...
    instance = ec2.run_instances(
        ...
        KeyName='TMKeyPrinceFrontend',  # Replace with your key pair name
        SecurityGroupIds=[sg_id],
        TagSpecifications=[{
            'ResourceType': 'instance',
            'Tags': [{'Key': 'Name', 'Value': 'TM-Prince-Frontend'}]
        }]
    )['Instances'][0]
    instance_id = instance['InstanceId']
...
try:
    ...
    # First check for a tagged EIP with name "FrontendEIP"
    for addr in addresses['Addresses']:
        if 'Tags' in addr:
            for tag in addr['Tags']:
                if tag['Key'] == 'Name' and tag['Value'] == 'FrontendEIP':
                    eip_allocation = addr
                    break
        if eip_allocation:
            break
            ...
            if 'InstanceId' not in addr:
                ...
                try:
                    ec2.create_tags(
                        ...
                        Tags=[{'Key': 'Name', 'Value': 'FrontendEIP'}]
                    )
                    print(f"Tagged existing EIP {addr['PublicIp']} as FrontendEIP")
    ...
    if eip_allocation:
        ...
        try:
            ...
            print("Elastic IP tagged: FrontendEIP")
        except ClientError as e:
